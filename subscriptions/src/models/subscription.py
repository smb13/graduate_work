...
from sqlalchemy import Column, Date, Enum, ForeignKey, Identity, Integer, String, UniqueConstraint
...
class SubscriptionType(TimestampMixin, Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<SubscriptionType name:{self.name}, description: {self.description}, "
...
class UserSubscription(IdMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("type_id", "user_id", "payment_method_id"),)
    ...
    def __repr__(self) -> str:
        return (
            f"<UserSubscription type_id:{self.type_id}, user_id: {self.user_id}, "
